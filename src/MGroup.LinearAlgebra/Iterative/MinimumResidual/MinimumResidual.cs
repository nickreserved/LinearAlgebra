...
namespace MGroup.LinearAlgebra.Iterative.MinimumResidual
{
	...
	public class MinRes
    {
        ...
        ///     <paramref name="rhsVector"/>.<see cref="IReadOnlyVector.Length"/> == 
        ...
        public (IVector solution, MinresStatistics stats) Solve(IMatrixView matrix, IReadOnlyVector rhsVector, double shift = 0.0)
		...
		/// <param name="shift">A scalar parameter that controls the deviation of (A - s*I) * x = b from A * x = b.</param>
		public (IVector solution, MinresStatistics stats) Solve(IMatrixView matrix, IReadOnlyVector rhsVector, 
		...
		///     <paramref name="matrix"/>.<see cref="IBounded2D.NumRows"/>.</param>
		/// <param name="shift">A scalar parameter that controls the deviation of (A - s*I) * x = b from A * x = b.</param>
		public (IVector solution, MinresStatistics stats) Solve(ILinearTransformation matrix, IReadOnlyVector rhsVector, 
            double shift = 0.0)
        ...
        private (IVector solution, MinresStatistics stats) SolveInternal(ILinearTransformation A, IReadOnlyVector b, 
            IPreconditioner M, double shift)
        {
            ...
            double rnorm = 0.0;
            double ynorm = 0.0;
            IVector x = b.CreateZeroWithSameFormat();
            ...
            IVector w = b.CreateZeroWithSameFormat();
            IVector w2 = b.CreateZeroWithSameFormat();
            IVector r2 = r1.Copy();
        ...
        private static void CheckSymmetricMatrix(ILinearTransformation A, IReadOnlyVector y, IReadOnlyVector r1)
        {
            IVector w = y.CreateZeroWithSameFormat();
            A.MultiplyIntoResult(y, w);
            IVector r2 = y.CreateZeroWithSameFormat();
        ...
        private void CheckSymmetricPreconditioner(IPreconditioner M, IReadOnlyVector y, IReadOnlyVector r1)
        {
            IVector r2 = y.CreateZeroWithSameFormat();
            M.Apply(y, r2);
        ...
        private static void WriteIterationData(ILinearTransformation A, IReadOnlyVector b, double shift, IReadOnlyVector x, int iter,
            ...
            int istop, double epsx, double epsr)
        {
            ...
            if (printThisOnce)
            {
                ...
                if (debug)
                {
                    ...
                    // vv = b - (A - shift * I) * x
                    IVector vv = b.CreateZeroWithSameFormat();
                    ShiftedMatrixVectorMult(A, x, vv, shift);
                    vv.SubtractIntoThis(b);

                    // ww = (A - shift * I) * vv = "Ar"
                    IVector ww = b.CreateZeroWithSameFormat();
        ...
        private class LocalReorthogonalizer
        {
            ...
            private readonly LinkedList<IReadOnlyVector> store;
            ...
            internal LocalReorthogonalizer(int order, int localSize)
            {
                ...
                this.store = new LinkedList<IReadOnlyVector>();
            }
            ...
            /// </summary>
            /// <param name="v"></param>
            internal void StoreDirection(IReadOnlyVector v)
            {
